...
```{r}
df <- read.csv("original_raw_all_cohorts.csv")
nrow(df)
```
...
# TREATMENT 2 = 2 (ENTER IN A LOTTERY FOR $200)
...
write.csv(df_final, "treatment_assignment.csv", row.names = FALSE)
...
```{r}
write.csv(df_rest,"remaining_emails.csv", row.names = FALSE)
```
