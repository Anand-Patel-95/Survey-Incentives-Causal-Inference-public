...
```{r}
#Group 0 vs. Group 1
df01 <- df1[treatment_group %in% c(0,1)]
...
stargazer(
  ...
  omit = 'cohort',
  header=F,
  notes = "Note: Uses Robust Standard Errors. Cohort included but omitted in model 2 and 3."
  )
```
...
```{r}
df12 <- df1[treatment_group %in% c(1,2)]
...
names(lrt1$coefficients) = c("5 Incentive", "100 Lottery", "No Incentive")
lrt2 <- df1[ , lm(response ~ relevel(factor(treatment_group_name), ref='$5 Incentive') + factor(cohort))]
names(lrt2$coefficients) = c("5 Incentive", "100 Lottery", "No Incentive", "Cohort1", "Cohort2", "Cohort3", "Cohort4")
lrt3 <- df1[ , lm(response ~ relevel(factor(treatment_group_name), ref='$5 Incentive') + in_PST + factor(cohort))]
names(lrt3$coefficients) <- c("5 Incentive", "100 Lottery", "No Incentive","in_PST", 'Cohort1', 'Cohort2', 'Cohort3', 'Cohort4')
